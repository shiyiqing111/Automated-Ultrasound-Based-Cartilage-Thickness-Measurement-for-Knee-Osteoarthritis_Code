...
# 当前文件：US_new/Model/optimization/seg_length_regression_resnet18_physio.py
# 项目根目录（有 config.py 的那个）：US_new/Model
PROJECT_ROOT = os.path.dirname(os.path.dirname(__file__))

# 确保 PROJECT_ROOT 在 sys.path 里
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from config import *   # 现在可以正常用了
...
print(">>> running seg_length_regression_resnet18_physio.py")
...
# --------------------------
# 计算 3 段长度（像素）的小函数
# 不依赖 geometry 里的 seg_lengths_from_coords，避免版本不一致
# --------------------------
...
# 转 cm（用你现成的像素转厘米系数），这里不做线性校正，保持简单
...
PRED_DIR_OPT = PRED_DIR / "seg_length_models_physio"
...
for seg in segment_order:
    ...
    for model_name, base_model in models.items():
        ...
        out_pred_path = PRED_DIR_OPT / f"{model_name}_resnet18_{seg}_length_pred_physio.xlsx"
...
out_summary = Path(RESULT_DIR) / "segment_length_models_resnet18_physio.xlsx"
